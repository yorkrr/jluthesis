...
\begin{equation*}
\rho_{\text{mixed}}=\sum_{a=1}^N p_a\rho_a ,\quad \sum_{a=1}^N p_a=1,
\tag{2.1}
\end{equation*}
...
\begin{equation*}
N_x^+=\int_{0}^{x_{\text{max}}} \frac{1}{\sigma} \frac{ \text{d}\sigma}{ \text{d}x}  \text{d}x, \quad N_x^-=\int_{x_{\text{min}}}^{0} \frac{1}{\sigma} \frac{ \text{d}\sigma}{ \text{d}x}  \text{d}x.
\tag{2.20}
\end{equation*}
当$x=\cos  \theta_i^ {\mathcal{A}}   \cos  \theta_j^ {\mathcal{B}}$  时，(2.18)式左侧可用不对称度表示，从而得到自旋关联矩阵的计算式
\begin{equation*}
C_{ij}=\frac{4}{\kappa^ {\mathcal{A}}   \kappa^ {\mathcal{B}}  } (A_{\cos  \theta_i^ {\mathcal{A} }  \cos  \theta_j^ {\mathcal{B} } }).
\tag{2.21}
\end{equation*}
...
基向量的选择会影响衰变产物的动量方向与坐标轴之间的夹角的计算。通常的做法是在$\tau \bar{\tau}$ 系统的质心参考系下选择固定的$\{ \hat{x} , \hat{y}, \hat{z} \}$基，其中$\hat{z}$ 沿着对撞机的束流方向，而$\hat{x}$和$\hat{y}$固定在垂直于$\hat{z}$方向的平面上。本研究使用螺旋度基：在$\tau \bar{\tau} $系统的质心参考系下定义$\{ \hat{r} , \hat{k}, \hat{n} \}$基，其中$\hat{k}$沿着$\tau^+$的出射方向，并有
\begin{equation*}
\begin{aligned}
\hat{r} &=\frac{1}{\sin \theta } (\hat{z} -\cos  \theta \hat{k} ), \\
\hat{n} &=\hat{r} \times \hat{k},
\end{aligned}
\tag{2.22}
\end{equation*}
\noindent 其中$\cos  \theta =\hat{k} \cdot \hat{z}$，$\hat{r}$方向与$\hat{z}$ 在垂直于$\hat{k}$方向的分量方向一致。参考文献[8]中的图1直观展示了这两种基的定义方式。
\begin{figure}[htb]
\centering
...
对于$pp \to \tau \bar{\tau}$过程，中间媒介子可能是Z玻色子或虚光子。而在Z玻色子的所有衰变通道中，$\mathrm{Z} \to \tau \bar{\tau}$的分支比为3.37\%[18]。
